    ...
    var SearchUI = {
        ...
        renderResults: function(query) {
            ...
            var html = this.results.map(function(result, index) {
                ...
                return '<div class="search-result' + (index === 0 ? ' selected' : '') + '" data-slug="' + self.escapeHtml(result.slug) + '" data-index="' + index + '">' +
                    '<div class="search-result-title">' + highlighted + '</div>' +
                    (preview ? '<div class="search-result-preview">' + preview + '</div>' : '') +
                    '<div class="search-result-path">' + self.escapeHtml(result.slug) + '</div>' +
                '</div>';
            }).join('');
