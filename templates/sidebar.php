...
function renderTree(array $items, string $currentPath, string $parentSlug = '', int $depth = 0): void
{
    foreach ($items as $item):
        ...
        // For JavaScript onclick handlers, escape properly
        $itemNameJs = jsEscape($item['name']);
        $itemSlugJs = jsEscape($item['slug']);
        ...
        <div class="sidebar-section<?= $navOnlyClass ?>">
            <div class="sidebar-section-header<?= $navOnlyClass ?>">
                ...
                <span class="sidebar-item-actions admin-only" style="display: none;">
                    <button class="sidebar-action-btn" onclick="event.stopPropagation(); AdminEditor.showCreateModal('<?= $itemSlugJs ?>')" title="Add new">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                    </button>
                    <button class="sidebar-action-btn" onclick="event.stopPropagation(); AdminEditor.showRenameModal('<?= $itemSlugJs ?>', '<?= $itemNameJs ?>')" title="Rename">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                    </button>
                    <button class="sidebar-action-btn" onclick="event.stopPropagation(); AdminEditor.showDeleteModal('<?= $itemSlugJs ?>', '<?= $itemNameJs ?>')" title="Delete">
        ...
        <div class="sidebar-link-wrapper">
            ...
            <a href="/docs/<?= $itemSlug ?>" class="sidebar-link<?= $isActive ? ' active' : '' ?>">
                ...
                <span class="sidebar-item-actions admin-only" style="display: none;">
                    ...
                    <button class="sidebar-action-btn" onclick="event.preventDefault(); event.stopPropagation(); AdminEditor.showRenameModal('<?= $itemSlugJs ?>', '<?= $itemNameJs ?>')" title="Rename">
                    ...
                    <button class="sidebar-action-btn" onclick="event.preventDefault(); event.stopPropagation(); AdminEditor.showDeleteModal('<?= $itemSlugJs ?>', '<?= $itemNameJs ?>')" title="Delete">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                    </button>
                </span>
            </a>
